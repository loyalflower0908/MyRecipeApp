...
<br>
      ...
      해당 프로젝트도 유튜브에서 MVVM 패턴이 적용된 앱을 따라 만들어보며 공부한 이후에 제작했지만,
    
      개념적으로 부족했던 것 같다.
    
      현재는 Repository의 구현체(RepositoryImpl)에 직접 의존하고 있는 부분이 있어서...
    
      의존성 역전 원칙 DIP 원칙에 위배된다는지...
    
      -> 고수준 모듈인 ViewModel이 저수준 모듈인 Repository의 구현체에 직접 의존하기 때문에
    
      Repository의 Interface만 알도록 해야한다..! 그래서 수정을 했지만...
      
      usecase를 통해 간접 접근 하는 방식이 더 나은 방법이라고 한다!
...
2025.01.18 / Dagger 의존성 및 Repository 바인딩 수정
